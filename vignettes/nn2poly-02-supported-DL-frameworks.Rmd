---
title: "02 - Supported DL frameworks"
author: "Pablo Morala"
output:
...
date: "2023-12-11"
vignette: >
  %\VignetteIndexEntry{02 - Supported DL frameworks}
...
> *Note*: Using the provided constraints when training a model increaes the number of needed epochs to converge, which is an expected trade off between learning speed and control over the weights for easier explainability.
...
> *Note*: Our implementation differs sligthly from the constraints provided by `keras` because in our implementation we join the bias with the rest of the weights while `keras` default constraints allow only for bias or kernel constraints. Our implementation uses a custom callback that is applied at the end of each batch.
...
![](figure/nn2poly-02-keras-nn_uncon-train-1.png)
...
![](figure/nn2poly-02-keras-nn_con-train-1.png)
...
plot_diagonal(x_axis =  prediction_nn_uncon, y_axis =  test_y, xlab = "Unconstrained NN prediction", ylab = "Original Y")
```

![](figure/nn2poly-02-keras-comparison-y-nn_uncon-1.png)
...
# Diagonal plot implemented in the package to quickly visualize and compare predictions
plot_diagonal(x_axis =  prediction_nn_con, y_axis =  test_y, xlab = "Constrained NN prediction", ylab = "Original Y")
```

![](figure/nn2poly-02-keras-comparison-y-nn_con-1.png)
...
prediction_poly_uncon <- eval_poly(x = test_x, poly = final_poly_uncon)
prediction_poly_con <- eval_poly(x = test_x, poly = final_poly_con)

plot_diagonal(x_axis =  prediction_nn_uncon, y_axis =  prediction_poly_uncon, xlab = "NN prediction", ylab = "Polynomial prediction") + ggplot2::ggtitle("Polynomial for nn_uncon")
```

![](figure/nn2poly-02-keras-polynomial-prediction-1.png)

```r

plot_diagonal(x_axis =  prediction_nn_con, y_axis =  prediction_poly_con, xlab = "NN prediction", ylab = "Polynomial prediction") + ggplot2::ggtitle("Polynomial for nn_con")
```

![](figure/nn2poly-02-keras-polynomial-prediction-2.png)
...
```r
plot_n_important_coeffs(final_poly_uncon, n_important_coeffs = 8)
```

![](figure/nn2poly-02-keras-n-important-1.png)

```r
plot_n_important_coeffs(final_poly_con, n_important_coeffs = 8)
```

![](figure/nn2poly-02-keras-n-important-2.png)


## `luz`/`torch`

In this section we will show how to use `nn2poly` with a `torch` neural network, built with its higher level API `luz`, and how to impose the needed weight constraints during training. 
...
In this framework, we need to input our data as a torch dataset
...
data_full <- torch_dataset(train_x, train_y)
...
# Data loader
torch_data <- list(
  train = torch::dataloader(data_train, batch_size = 50, shuffle = TRUE),
  valid = torch::dataloader(data_val, batch_size = 50)
)
```
...
Now we will set the structure of our neural networks using a sequential model in `torch`. This following function will be used to create both the constrained and unconstrained neural networks in the `torch` example.
...
![](figure/nn2poly-02-torch-nn_uncon-train-1.png)
...
![](figure/nn2poly-02-torch-nn_con-train-1.png)
...
![](figure/nn2poly-02-torch-comparison-y-nn_uncon-1.png)
...
![](figure/nn2poly-02-torch-comparison-y-nn_con-1.png)
...
![](figure/nn2poly-02-reg-polynomial-prediction-1.png)
...
![](figure/nn2poly-02-reg-polynomial-prediction-2.png)

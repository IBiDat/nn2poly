...
This vignette presents the direct usage of the package with a simple case. For additional tools to build neural networks with the needed conditions that ensure the correct theoretical behavior (constraining the weights and biases), check the VIGNETTE 2 that also uses the extension package `nn2polytools`.
...
#> Warning in is.na(d): is.na() applied to non-(list or vector) of type 'environment'
...
![plot of chunk history](figure/nn2poly-02-history-1.png)
...
![plot of chunk comparison-y-nn](figure/nn2poly-02-comparison-y-nn-1.png)
...
```r
coeffs[1:7]
#> [1] -0.60875043 -0.08855389  0.08102358 -0.01863074  0.06359143 -0.11998199  0.37702593
```
...
![plot of chunk comparison-pr-nn](figure/nn2poly-02-comparison-pr-nn-1.png)
...
```r
plot <- plot_taylor_and_synpatic_potentials(data = train,
                                    weights_list = nn_weights,
                                    af_string_list = af_string_list,
                                    q_taylor_vector = q_taylor_vector)
```
...
![plot of chunk unnamed-chunk-12](figure/nn2poly-02-unnamed-chunk-12-1.png)
...
![plot of chunk unnamed-chunk-12](figure/nn2poly-02-unnamed-chunk-12-2.png)

...
It is advisable to always check that the predictions obtained with the new polynomial are close to the original NN predictions (and in case they differ, we can also try to find why by checking the Taylor expansions). To help with that, some functions are included that allow us to plot the results.
...
> *Note*: This plot is left as an internal function so it can be used as a quick check of the polynomial behavior, but might be removed later on. Any simple scatter plot comparing NN predictions with Polynomial predictions can be used for this purpose.
...
### Polynomial Coefficients Barplot
...
### 2nd Order Coefficients Heatmap
...
```{r reg-beeswarm, eval=requireNamespace("ggbeeswarm", quietly = TRUE)}
plot(final_poly,
     type = "beeswarm",
     newdata_monomials = prediction_monomials,

...
```{r}
#library(nn2poly)
reticulate::use_condaenv("py310-tf211", required = TRUE)
set.seed(42)
```
...
### Trying a Chebyshev approximation

The default call to `nn2poly()` uses Taylor expansions for the activation
functions. To test the Chebyshev alternative on the same constrained `keras`
model, only the approximation arguments need to be changed. The interval
`c(-1, 1)` matches the data scaling and the weight-constraint setup used in
this vignette.

```{r keras-chebyshev-test, eval=FALSE}
# Chebyshev polynomial for the constrained NN
final_poly_con_cheb <- nn2poly(
  object = nn_con,
  max_order = 5,
...
# Same monomial structure as the default Taylor polynomial
identical(final_poly_con$labels, final_poly_con_cheb$labels)

# Compare approximation error against the constrained NN predictions
prediction_poly_con_cheb <- predict(
  object = final_poly_con_cheb,
  newdata = test_x
)

c(
  taylor_rmse = sqrt(mean((as.vector(prediction_nn_con) -
                             as.vector(prediction_poly_con))^2)),
  chebyshev_rmse = sqrt(mean((as.vector(prediction_nn_con) -
                                as.vector(prediction_poly_con_cheb))^2))
)

nn2poly:::plot_diagonal(
  x_axis = prediction_nn_con,
  y_axis = prediction_poly_con_cheb,
  xlab = "NN prediction",
  ylab = "Chebyshev polynomial prediction"
) + ggplot2::ggtitle("Chebyshev polynomial for nn_con")

plot(final_poly_con_cheb, n = 8)
```


```{r}
final_poly_con_cheb$values
final_poly_con$values
```

```{r keras-chebyshev-unconstrained-test, eval=FALSE}
# Chebyshev polynomial for the constrained NN
final_poly_uncon_cheb <- nn2poly(
  object = nn_uncon,
  max_order = 3,
  approximation = "chebyshev",
  chebyshev_interval = c(-2,2)
)

# Same monomial structure as the default Taylor polynomial
identical(final_poly_con$labels, final_poly_uncon_cheb$labels)

# Compare approximation error against the constrained NN predictions
prediction_poly_uncon_cheb <- predict(
  object = final_poly_uncon_cheb,
  newdata = test_x
)

c(
  taylor_rmse = sqrt(mean((as.vector(prediction_nn_uncon) -
                             as.vector(prediction_poly_uncon))^2)),
  chebyshev_rmse = sqrt(mean((as.vector(prediction_nn_uncon) -
                                as.vector(prediction_poly_uncon_cheb))^2))
)

nn2poly:::plot_diagonal(
  x_axis = prediction_nn_uncon,
  y_axis = prediction_poly_uncon_cheb,
  xlab = "NN prediction",
  ylab = "Chebyshev polynomial prediction"
) + ggplot2::ggtitle("Chebyshev polynomial for nn_uncon")

plot(final_poly_uncon_cheb, n = 8)
```

```{r}
nn2poly:::plot_taylor_and_activation_potentials(final_poly_uncon_cheb,)
```

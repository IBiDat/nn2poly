...
With the obtained polynomial coefficients, we can use them to predict the response variable $Y$ using the polynomial. This can be done using `predict()` on the output of `nn2poly` (object with class `"nn2poly"`) together with the desired values for the predictor variables.
...
# as we are predicting the output of our final polynomial for each observation.
...
```r
plot(final_poly, n = 8, type = "bar", variable_names = feature_names)
```

![](includes/nn2poly-01-reg-n-important-1.png)

The heatmap focuses on second-order coefficients. Diagonal cells correspond to squared terms such as $x_1^2$, while off-diagonal cells correspond to pairwise interactions such as $x_2x_3$.


```r
plot(final_poly, type = "heatmap", variable_names = feature_names)
```

The monomial predictions computed above give an exact decomposition of each polynomial prediction into its evaluated terms. The local contribution plot redistributes each evaluated term across the variables that appear in it, proportionally to their multiplicity in the term. This is useful as a feature-level summary for one observation, while keeping in mind that interaction terms are shared by construction.


```r
plot(final_poly,
     type = "local_contributions",
     newdata_monomials = prediction_monomials,
     observation_index = 1,
     max_order_to_display = 3,
     variable_names = feature_names)
```

The waterfall plot keeps the decomposition at the term level. The bars start at the intercept, add the largest term contributions for the selected observation, aggregate the remaining terms into `(Other)`, and end at the polynomial prediction.
...
Across a data set, a beeswarm plot shows how the contribution of the most relevant terms changes from observation to observation. The color can be tied to any original feature, which helps identify how the feature values relate to the evaluated term contributions.
...
plot(final_poly,
     type = "beeswarm",
     newdata_monomials = prediction_monomials,
     original_feature_data = test_x,
     color_by_feature = 2,
     top_n_terms = 10,
     variable_names = feature_names)
```

The response surface varies two selected features over the range observed in `original_feature_data` and fixes the remaining features at their sample means. This gives a direct view of the fitted polynomial output over a two-dimensional slice of the predictor space.


```r
plot(final_poly,
     type = "interaction_surface",
     feature_pair = c("x2", "x3"),
     original_feature_data = test_x,
     grid_resolution = 30,
     variable_names = feature_names)
```

Finally, the interaction network summarizes which variables appear together in interaction terms. With `metric_network = "coefficient_abs"`, edge strength is based on coefficient magnitude. With `metric_network = "mean_monomial_abs"`, edge strength is based on the average absolute evaluated contribution across the data.
